...
const state={modelVersion:'v1',crop:'tomato',input:null,result:null,busy:false,generation:0,history:[],evaluation:null,filter:'all',query:'',error:''};
let toastTimer;
...
function scanner(){
  $('#content').innerHTML=heading('THE LEAF SCANNER','Good care starts with a closer look.','Choose your crop and check a leaf photograph.',tag('3 crops · research models'))+
  `<div class="mode-note"><span class="mode-label">RESEARCH MODEL</span><p>Uploaded photos run through a trained model on your device. Reference demos remain simulated. V1 external benchmark: 25% accuracy on 36 images. V2 is an experimental development model with a spider-mite damage class and an unsupported-condition class. Research use only.</p><a href="#project">How it works ${icon('arrow')}</a></div>
  ...
  <div class="input-body"><div class="crop-line"><span class="crop-icon">${icon('leaf')}</span><div><label for="crop-select">Select crop</label><select id="crop-select">${Object.entries(crops).map(([key,crop])=>`<option value="${key}" ${state.crop===key?'selected':''}>${crop.name}</option>`).join('')}</select><small>${crops[state.crop].scientific}</small></div>${tag('Local prediction','neutral')}</div><p class="muted">Supported classes: ${crops[state.crop].classes.map(c=>c[1]).join(' · ')}${state.modelVersion==='v2'&&state.crop==='tomato'?' · Spider-mite damage (experimental)':''}. Research predictions require confirmation.</p>
  <label for="model-select">Research model</label><select id="model-select"><option value="v1" ${state.modelVersion==='v1'?'selected':''}>V1 · original nine-class benchmark</option><option value="v2" ${state.modelVersion==='v2'?'selected':''}>V2 · experimental field-style training + mite damage</option></select><p class="muted">${state.modelVersion==='v2'?'Adds tomato spider-mite damage and an explicit unsupported-condition output. Development validation only; no proven farm accuracy. This does not identify insects or reliably detect non-leaf images.':'Original model with published held-out and external test results.'}</p>
  ...
  $('#model-select').addEventListener('change',e=>{clearInput();state.modelVersion=e.target.value;scanner();});
  const dz=$('#dropzone');
  ...
  try{
    ...
    if(input.kind==='sample')Object.assign(report,{key:input.key,confidence:cases[input.key].score,probabilities:cases[input.key].probabilities,disclaimer:'Simulated output for a labeled reference. No inference was performed.'});
      ...
      if(checks.every(c=>c.ok)){
        const prediction=await predict(input.image,crop,state.modelVersion);
        Object.assign(report,prediction,{kind:'model',disclaimer:'Research prediction from a limited model. V2 adds tomato spider-mite damage appearance, not insect identification. Non-leaf images and unsupported diseases can receive confident but incorrect results. Seek agricultural confirmation before acting.'});
      }
    }
...
function saveReport(report=state.result){if(!report)return;saveFile(`leaflens-${report.kind}-${report.id.slice(0,8)}.txt`,reportText(report));toast('Report saved with its model version and limitations.');}
...
function project(){
  ...
  <div class="project-grid"><section class="panel panel-padding"><h2>Training and evaluation</h2><p>A compact CNN and a pretrained MobileNetV2 are compared using the same validation split. The selected model is calibrated on validation data and evaluated on the reserved test set.</p><p>8,489 images · 5,919 training · 1,285 validation · 1,285 testing. Tomato and potato use recorded leaf groups. Maize splits are provisional image-level splits.</p><a class="btn secondary" href="#evaluation">Explore measured results</a></section><section class="panel panel-padding"><h2>What the results do not establish</h2><p>A separate PlantDoc check scored 25% on 36 supported-disease images; 22 of 41 unsupported conditions passed the confidence/crop rule. Independent phone and farm trials remain outstanding. Backgrounds may influence predictions. Healthy potato has only 24 test images. Confidence does not establish that a photo contains a supported leaf.</p><p>V2 adds a tomato spider-mite damage class. It does not identify insects, other pests, or prescribe pesticides.</p></section></div>
  <section class="panel panel-padding"><h2>V2 experiment: broader training, honest limits</h2><p>V2 learns from PlantDoc training photographs, an explicit unsupported-condition class, and PlantVillage tomato spider-mite damage. V1 remains available for comparison. V2 scores are development validation used for selection, not a new untouched test or proof of field accuracy.</p><p>On the same 89 supported PlantDoc development images, V1 scored 20.2% and V2 scored 38.2%. V2 accepted 17 of those 89 images. Unsupported inputs accepted: V1 232/324, V2 6/324. V2 was selected using this development data, and its stricter threshold also affects acceptance; this is not an independent field test.</p><p>Non-leaf rejection and expert-confirmed farm validation are still unverified. No real farm photos or expert labels have been supplied.</p><a href="model-v2/validation_metrics.json">V2 development metrics</a> · <a href="model-v2/model-card.md">V2 scope and training report</a> · <a href="field-validation.md">Field validation protocol</a></section>
