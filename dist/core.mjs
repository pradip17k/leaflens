...
export function qualityChecks(stats,width,height) {
  return [
    {label:'Image resolution',value:`${width} × ${height} px`,ok:Math.min(width,height)>=224,detail:Math.min(width,height)>=224?'Enough pixels for a 224 × 224 model input.':'Try an image at least 224 pixels on each side.'},
...
export function parsePredictions(text) {
  ...
  return rows.map((row,i)=>{const pair=cells(row);if(pair.length!==2||pair.some(v=>!LABELS.includes(v)))throw Error(`Row ${i+2}: use healthy, early or septoria in both columns.`);return {actual:pair[0],predicted:pair[1]};});
}
